...
describe("NFT", async function () {
  it("should be able to mint, transferFrom, burn. And it should return appropriate name, symbol, totalSupply, tokenURI, ownerOf, balanceOf", async function () {
    ...
    expect(await nft.tokenURI(0)).to.equal(
      "https://asia-northeast1-nft-survey.cloudfunctions.net/api/v1/tokens/0"
    );
    ...
    expect(await nft.tokenURI(1)).to.equal(
      "https://asia-northeast1-nft-survey.cloudfunctions.net/api/v1/tokens/1"
    );
    ...
    const transfer1FromSignerToAddressTx = await nft
      .connect(signer)
    ...
    expect(await nft.totalSupply()).to.equal(1);
    expect(nft.ownerOf(0)).to.revertedWith(
      "ERC721: owner query for nonexistent token"
    );
    expect(nft.tokenURI(0)).to.revertedWith(
    ...
    expect(await nft.tokenURI(2)).to.equal(
      "https://asia-northeast1-nft-survey.cloudfunctions.net/api/v1/tokens/2"
    );
    expect(await nft.balanceOf(badSigner.address)).to.equal(2);

    // transfer token(tokenId = 2) from badSigner.address to signer.address
    const transfer2FromBadSignerToSignerAddressTx = await nft
      .connect(badSigner)
    ...
    expect(nft.connect(badSigner).mint(signer.address)).to.revertedWith(
